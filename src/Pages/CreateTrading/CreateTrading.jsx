...
export default function CreateTrading() {
  ...
  const { title, description, publishDate, publishBy, img } = trading
  const [clearField, setClearField] = useState(trading)
  ...
  const config = {
    placeholder: 'Start writing your amazing trading...'
  }
  ...
  const getUserDetail = async() => {
    ...
    'Content-Type': 'application/json'
      ...
      .then((response => {
        trading.publishBy = response.data.user.userName
  ...
  const handleInputChange = (e) => {
    ...
    setTrading({ ...trading, [e.target.name]: e.target.value })

  }
  const handleInputState = (name, value) => {
    setTrading((prev) => ({ ...prev, [name]: value }));

  }

  const resetForm = () => {
    setTrading(clearField)
    setContent('')
  }

  const registerTrading = async() => {
    await tradingServices.createTrading(trading)
    .then((response=>{
        console.log(response.data.message)
        console.log(trading)
        console.log("Redirecting to trading portal..!")
        setTimeout(() => {navigate('/trading')}, 2000);
    }))
    .catch((response) => {
        // console.log(error);
        console.log(response.data.message)
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    trading.publishDate = currentDate
    // if (title && description && publishDate && publishBy && img) {
      registerTrading(trading)
      setNotify({
        isOpen: true,
        message: 'Trading Added Succesfully',
        type: 'success'
      })
              ...
              <Grid item xs={8}>
                <Card sx={{ height: 600, maxWidth: '100%' }}>
                  ...
                  <Stack direction="column">
                    ...
                    <TextField id="outlined-basic" label="Blog Title" variant="outlined" name="title" autoComplete='off' value={trading.title} onChange={handleInputChange} sx={{ marginTop: '12px' }} />
  ...
  /> */}
                    ...
                    <JoditEditor
                      ...
                      onChange={newContent => {trading.description = newContent}}
                    />
